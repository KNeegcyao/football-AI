package com.soccer.forum.service.modules.match.service.impl;
...
import com.soccer.forum.service.modules.match.mapper.TeamMapper;
...
public class PlayerSyncServiceImpl implements PlayerSyncService {
    ...
    static {
        ...
        POSITION_MAP.put("Attacking Midfield", "攻击型中场");
            ...
            if (personData != null) {
                ...
                if (personData.hasNonNull("dateOfBirth")) {
                    try {
                        ...
                        log.warn("解析出生日期失败: {}", player.getName());
                    }
                }
                ...
                if (personData.hasNonNull("marketValue")) {
                    ...
                    try {
                        ...
                        log.warn("解析市场价值失败: {} - {}", player.getName(), mvStr);
                    }
                }
    ...
    public PlayerSyncServiceImpl(PlayerMapper playerMapper, 
        ...
        this.sportApiClient = sportApiClient;
    ...
    public JsonNode getPlayerDetailJson(Long id) {
        ...
        // 自动数据补充: 如果关键字段缺失且有 API ID（且不是刚刚更新）
        if (player.getApiId() != null && (player.getBirthPlace() == null || player.getContractUntil() == null 
            ...
            if (shouldSync) {
                try {
                    log.info("检测到球员数据不全 (id={}), 尝试自动同步...", id);
    ...
    public String syncTeamPlayers(Long teamId, Integer apiTeamId, Integer season) {
        // 如果 season 为null，默认使用2023 (当前赛季)
        if (season == null) {
            season = 2023; // 默认为2023，或者应从系统配置获取
        }
    ...
    private void processPlayerSync(Long teamId, JsonNode item) {
        ...
        // 尝试通过名字和球队匹配 (如果 API-Football 和 Football-Data ID 不一致导致重复创建)
        if (player == null) {
        ...
        // 尝试通过号码匹配 (假设名字拼写不一致，但都在 API-Football vs Football-Data)
        ...
        // 更新基础信息
        player.setName(name);
        player.setDisplayName(lastname + " " + firstname); // 简单拼接，实际可用更好的API
        ...
        if (isNew) {
            // 如果球员没有身价估值，我们暂不创建 (通常只有 Football-Data 才有比较好的身价数据)
            // 或者这些球员是边缘球员 (如果 Football-Data 也没同步到)
            // 我们只要有身价的或者主力
            if (player.getMarketValue() == null) {
                // API-Football 通常没有身价，但是它提供了详细的记录(通常由 Football-Data 补充身价)
                // 那么如果球员已经是边缘球员，且 Football-Data 也没同步到
                // 我们不需要没有身价的普通球员
                log.warn("忽略无身价球员(API-Football): {} (ID: {})", name, apiId);
                return;
            }
            playerMapper.insert(player);
        } else {
            // 更新时不覆盖 apiId (因为可能原本是 Football-Data 的ID)
            // 也不强制更新身价 (保留现有记录，因为身价来自 Football-Data 更准)
            playerMapper.updateById(player);
        }
    }
    ...
    public void syncMarketValuesAndStatus() {
        log.info("开始同步球员身价和状态...");
        // 伪代码: 获取所有 API 数据或循环更新
    }
    ...
    public JsonNode getPlayerFromSportApi(Integer playerId) {
        log.info("从 SportAPI (Mock/DB) 获取球员信息: id={}", playerId);
    ...
    public String syncPlayerFromSportApi(Integer sportApiId) {
        ...
        try {
            ...
            // 标记来源 sportapi (因为目前只使用 sportapi 数据更新)
    ...
    public String syncAllTeamsFromFootballData() {
        log.info("开始使用 Football-Data.org 同步所有球队球员...");
        ...
        try {
            ...
            for (java.util.Map.Entry<String, Integer> entry : FOOTBALL_DATA_TEAMS.entrySet()) {
                ...
                log.info("处理同步球队: {} (ID: {})", teamName, apiTeamId);
    ...
    private String syncFootballDataForTeam(Team localTeam, Integer apiTeamId) {
        ...
        if (squadData == null || !squadData.hasNonNull("squad")) {
            log.warn("无法获取球队阵容: {}", localTeam.getName());
        ...
        for (JsonNode pNode : squad) {
            ...
            if (player == null && jerseyNumber != null) {
                player = playerMapper.selectOne(new LambdaQueryWrapper<Player>()
                        ...
                        .last("LIMIT 1"));
                if (player != null) {
                    log.info("通过号码匹配到球员 {} -> {} (API Name: {})", player.getJerseyNumber(), player.getName(), pName);
                }
            }
            ...
            if (player == null) {
                ...
                // 2. 只有大号码且年龄很小 (通常是预备队提拔)
                // 规则: 号码 > 55 且年龄 < 19 -> 忽略
                if (jerseyNumber > 55 && age < 19) {
                    log.info("忽略年轻球员/预备队 {} (No. {}, Age {})", pName, jerseyNumber, age);
                    continue;
                }
            }
            ...
            if (player == null) {
                ...
                // 更新现有球员
                // 如果原本 source 没有 Football-Data，可以追加
                if (player.getSourceFrom() == null || !player.getSourceFrom().contains("Football-Data")) {
                     // 可选择是否追加标记
            ...
            if (isNew) {
                // 如果球员没有身价估值，我们暂不创建
                if (player.getMarketValue() == null) {
                    // 再次尝试确认是否能从其他途径获取 (目前只依赖 fillFootballDataDetails)
                    // 如果确实没有，就跳过
                    log.warn("忽略无身价球员 {} (ID: {})", pName, apiPlayerId);
                    continue;
                }
