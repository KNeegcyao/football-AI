...
public class AuthServiceImpl implements AuthService {
    ...
    public void register(LoginBody loginBody) {
        ...
        if (count > 0) {
            log.warn("注册失败: 用户{} 已存在", loginBody.getUsername());
        ...
        // 发送欢迎私信
         chatMessageService.sendMessage(1L, user.getId(), "欢迎加入懂球帝论坛！我是官方助手，有任何问题都可以向我反馈。", 1);
    }
    ...
    public void resetPassword(LoginBody loginBody) {
        try {
            log.debug("尝试重置密码: 用户{}", loginBody.getUsername());
